...
log = logging.getLogger("genpod.state_space_generator")
...
def get_updated_true_observation(true_observation: set[Predicate], state, action, grounding):
    ...
    if (s - s_a) & true_observation:
        log.debug(f"s-s_a: {state_to_string(s - s_a)}, true_observation: {state_to_string(true_observation)}")
        for obs in true_observation:
            for l in (s_a - s):
                log.debug(f"Checking if {obs.name} == {l.name}")
                if obs.name == l.name:
